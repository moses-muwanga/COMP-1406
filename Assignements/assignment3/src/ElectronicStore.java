...
public class ElectronicStore {
  ...
  private int curProducts, cartProducts;
  String name;
  Product[] stock, cartStock; //Array to hold all products
  double revenue;
  int sale;
  ...
  public ElectronicStore(String initName) {
    ...
    cartStock = new Product[MAX_PRODUCTS];
    curProducts = 0;
    sale = 0;
  }
  ...
  public boolean addProduct(Product newProduct) {
    if (curProducts < MAX_PRODUCTS) {
      ...
      cartStock[curProducts] = newProduct;
      curProducts++;
      cartProducts++;
      return true;
    }
  ...
  public void removeProduct(int index) {
    if (index >= 0 && index < curProducts) {
      stock[index] = null;
      for (int i = index + 1; i < curProducts; i++) {
